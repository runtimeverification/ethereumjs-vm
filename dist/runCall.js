...
module.exports = function (opts, cb) {
  ...
  function runClient(_callback) {
    ...
    client.on('data', function (data) {
      ...
      try {
        ...
        var shortData = new Uint8Array(Buffer.from(data).slice(4));
  ...
  function computeCallResult(query, _callback) {
    ...
    updateAccounts(modifiedAccounts, function () {
      var runState = {
  ...
  function updateAccounts(accountList, _callback) {
    async.series(accountList.map(function (account) {
      return function (_itemCallback) {
  ...
  function updateAccount(pbAccount, _done) {
    ...
    async.series([function (_callback) {
      ...
      async.series(storageUpdateList.map(function (item) {
        return putContractHelper(address, item);
      }), _callback);
    }], _done);
  }
  ...
  function fromLogEntries(logEntries) {
    ...
    for (var i = 0; i < logEntries.length; i++) {
      ...
      log[1] = [];
      logEntry.getTopicsList().forEach(function (element) {
        return log[1].push(Buffer.from(element));
      });
