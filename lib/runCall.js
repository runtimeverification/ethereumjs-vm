...
module.exports = function (opts, cb) {
  ...
  function runClient (_callback) {
    ...
    client.on('data', function (data) {
      var query
      try {
        query = msg_pb.VMQuery.deserializeBinary(data)
      } catch (err) {
        if (err.message === 'Assertion failed') {
          var shortData = new Uint8Array(data.length - 4)
          for (var i = 4; i < data.length; i++) {
            shortData[i - 4] = data[i]
          }
          query = msg_pb.VMQuery.deserializeBinary(shortData)
        }
      }
  ...
  function computeGetCode (query) {
    ...
    stateManager.getContractCode(Buffer(address), function (err, contractCode, compiled) {
      if (err) {
        console.log(err)
      }
      isCompiled = compiled
  ...
  function computeGetAccount (query) {
    ...
    stateManager.getAccount(new Buffer(address), function (err, fromAccount) {
      if (fromAccount.isEmpty()) { // FIX THIS (createdAddress = returnCode)
        if (salt) {
