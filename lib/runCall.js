...
// data is what the server sent to this socket
  ...
  function computeGetBlockhash(query){
    var getBlockhashObject = query.getGetblockhash()
    var offset = getBlockhashObject.getOffset()
  ...
  function computeGetStorageData(query) {
    ...
    storageReader.getContractStorage(Buffer(address), Buffer(offset), function(err, value){
      ...
      storageData = createStorageData(value)
      client.write(storageData)
    })
    // stateManager._getStorageTrie(Buffer(address), (err, trie) => {
    //   if (err) {
    //     console.log(err)
    //   }
    //   storageData = createStorageData(trie)
    //   client.write(storageData)
    // })
  }
  ...
  function createStorageData(data){
    storageData = new msg_pb.StorageData()
    storageData.setData(new Uint8Array(4))
    console.log(storageData)
    bytes = storageData.serializeBinary()
