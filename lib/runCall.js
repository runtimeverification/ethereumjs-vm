...
module.exports = function (opts, cb) {
  ...
  async.series([checkpointState,runClient,computeCallResult],function(){
    console.log('done')
  })
  ...
  function computeCallResult(query, done){
    ...
    var deletedAccounts = callResultObject.getDeletedaccountsList()
    var touchedAccounts = callResultObject.getTouchedaccountsList()
    ...
    updateAccounts(modifiedAccounts, done)
  }

  function updateAccounts(accountList, done){
    accountList.forEach(account => {
      updateAccount(account, done)
    })
  }

  function updateAccount(pbAccount, done){
    var address = Buffer(pbAccount.getAddress())
    var nonce = Buffer(pbAccount.getNonce())
    var newBalance = Buffer(pbAccount.getBalance())
    var code = Buffer(pbAccount.getCode())
    var storageUpdateList = pbAccount.getStorageupdatesList()

    async.series([function(_callback){
      ...
      function(_callback){
        stateManager.putContractCode(address, code, function(){
        console.log("Updated code for address", address.toString('hex'))
        _callback()
      })},
      function(_callback){
        storageUpdateList.forEach(item => {
          var key = Buffer(item.getOffset())
          var value = Buffer(item.getData())
          stateManager.putContractStorage(address, key, value, function(){
            console.log("Updated storage for address", address.toString('hex'))
            console.log("Key:", key.toString('hex'))
            console.log("Value:", parseInt(value.toString('hex'),16))
          })
        })
        _callback()
      }
  ...
  function fromLogEntries(logEntries){
    ...
    for(i = 0; i<logEntries.length; i++){
      ...
      var log = []
      log.length = 3
      log[0] = new Buffer(logEntry.getAddress())
      log[1] = []
      logEntry.getTopicsList().forEach(element => log[1].push(new Buffer(element)))
      log[2] = new Buffer(logEntry.getData())
      logList.push(log)
    }
    return logList
  }
    ...
    stateManager.getAccount(new Buffer(address), function (err, fromAccount) {
      if(fromAccount.isEmpty()){
        if (salt) {
          ...
          createdAddress = toAddress = ethUtil.generateAddress(caller, new BN(caller.nonce).toArray())
        }
      }
