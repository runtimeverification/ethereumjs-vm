...
module.exports = function (opts, cb) {
  ...
  function updateAccounts(accountList, _callback){
    ...
    accountList.forEach(account => {
      updateAccount(account, _callback)
    })
  }
    ...
    async.series(
      [function (_callback) {
        stateManager.getAccount(address, function (err, stateAccount) {
          stateAccount.nonce = Buffer(nonce)
          stateAccount.balance = Buffer(newBalance)
      ...
      function (_callback) {
        storageUpdateList.forEach(item => {
          ...
          stateManager.putContractStorage(address, key, value, function () {
            ...
            // storageReader.getContractStorage(address, key, (err, v) => {
            //   console.log("val", v)
            //   counter = counter + 1
            //   if (counter === length) {
            //     _done()
  ...
  function computeGetCode(query){
    ...
    stateManager.getContractCode(Buffer(address), function(err, c, comp){
      isCompiled = comp
      code = c
  ...
  function createEthereumConfig () {
    ...
    ethereumConfig.setAccountstartnonce(new Uint8Array([1]))
