...
module.exports = function (opts, cb) {
  ...
  function computeGetStorageData (query, _callback) {
    ...
    // Remove any zeroes prepended by KEVM
    address = chopZeroes(address)
    offset = chopZeroes(offset)
  ...
  function createStorageData (data) {
    ...
    dataArray = ensurePositive(dataArray)
    storageData.setData(dataArray)
  ...
  function createCallContext () {
    ...
    var value = Buffer.from(txValue)
    value = ensurePositive(value)
    callCtx.setCallvalue(new Uint8Array(value))
  ...
  function ensurePositive (buffer) {
    ...
    if (buffer[0] >= 0x80) {
      var c = new Uint8Array(1 + buffer.length)
      c[0] = 0
      c.set(buffer, 1)
      buffer = c
    }
